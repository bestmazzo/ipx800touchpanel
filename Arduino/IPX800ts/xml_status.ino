...
void parseXMLcommand( String &tagstr,  char *data){    
   ...
   //Serial.print(tagstr);Serial.print("->");Serial.println(data);
    if ( tagstr.startsWith("led"))  {
      if (ipx[curPoll].l[tagstr[3]-48]!= atoi(data)) ipx[curPoll].modL=true;
      ipx[curPoll].l[tagstr[3]-48]= atoi(data);
    }
    else if ( tagstr.startsWith("an")) {
       if ( ipx[curPoll].a[tagstr[2]-49] != atoi(data)) ipx[curPoll].modA=true;
       ipx[curPoll].a[tagstr[2]-49]=atoi(data);      
    }
    else if ( tagstr.startsWith("btn")) {
       if ( ipx[curPoll].d[tagstr[3]-48] != (data[0] == 'd')) ipx[curPoll].modD=true;
       ipx[curPoll].d[tagstr[3]-48]= (data[0] == 'd');      
    }
